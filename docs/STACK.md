...
**Why we used it:** Some browser extensions inject DOM nodes into form inputs _before_ React hydrates the page, causing hydration mismatch errors. Loading the form client-only means the SSR HTML has no inputs for extensions to target — error eliminated.
...
_Last updated: 2026-05. Tracks the state of the codebase on the `master` branch._
